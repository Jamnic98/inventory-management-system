...
export default function AddItemsModal(props) {
  ...
  return (
    <SimpleModal title="Add Items" isOpen={isOpen} closeModal={closeModal} style={modalStyle}>
      <Divider />
          ...
          return (
            <div key={item._id}>
              <ListItem>
                ...
                <ListItemText primary={item.name} secondary={`${item.room} ${item.location}`} />
                <IconButton
                  tabIndex={-1}
                  color="secondary"
                ...
                <span style={{ fontSize: '1.2em', margin: '0.3em' }}>{`${item.selected}`}</span>
                <IconButton
                  tabIndex={-1}
                  color="secondary"
