...
export default function MainTable(props) {
  ...
  const emptyRows =
    ROWS_PER_PAGE - Math.min(ROWS_PER_PAGE, filteredData.length - page * ROWS_PER_PAGE)
  ...
  const isSelected = (row) => itemsToModify.map((item) => item._id).indexOf(row._id) !== -1

  const setExpirationDate = (date) => {
    return parseInt(Date.parse(date)) !== 0 ? new Date(date).toLocaleDateString('en-GB') : '-'
  }
  ...
  return (
    <Box py={1}>
      <Box mx={1}>
        <ButtonGroup color="default" variant="text" size="large" aria-label="button group">
          <Button onClick={handleAddButton} endIcon={<AddIcon>add</AddIcon>}>
            add
          </Button>
          <Button onClick={handleEditButton} endIcon={<EditIcon>edit</EditIcon>}>
            edit
          </Button>
          <Button onClick={handleRemoveButton} endIcon={<DeleteIcon>remove</DeleteIcon>}>
            remove
          </Button>
        </ButtonGroup>
        <Button
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className={classes.tableCell} align="center">
                      <div className={classes.textContainer}>{row.quantity}</div>
                    </TableCell>
                    <TableCell className={classes.tableCell} align="center">
                      <div className={classes.textContainer}>{row.room}</div>
                    </TableCell>
                    <TableCell className={classes.tableCell} align="center">
                      <div className={classes.textContainer}>{row.location}</div>
                    </TableCell>
                    <TableCell className={classes.tableCell} align="center">
                    ...
                    <TableCell className={classes.tableCell} align="center">
                      <div className={classes.textContainer}>{row.lowStockAlert.toString()}</div>
                    </TableCell>
                  </TableRow>
                )
              })}
