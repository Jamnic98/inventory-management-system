...
export default function RoomLocationTree(props) {
  ...
  const createTree = (parentNode) => {
    const { id: nodeId, children, editing, label, layer, isSelected } = parentNode

    if (children.length === 0) {
      return (
        <ListItem key={nodeId} margin="dense" style={{ paddingLeft: getPadding(layer) }}>
          <ClickAwayListener mouseEvent="onClick" onClickAway={() => handleClickAway(nodeId)}>
            <div
      ...
      const { id: nodeId, layer, label, editing, isOpen, children, isSelected } = parentNode
      return (
        <div key={nodeId}>
          <ListItem
            margin="dense"
